  ...
  <div class="monitors">
    <div class="monitors-header">
      ...
      <div class="header-actions">
        <button @click="manualRefresh" class="btn btn-refresh">
          <i class="fas fa-sync-alt"></i>
          Refresh
        </button>
        <router-link to="/monitors/create" class="btn btn-primary">
    ...
    <div class="monitors-content">
      ...
      <div class="stats-cards" v-if="!monitorStore.loading">
        ...
        <div class="stat-card services" style="animation-delay: 0.2s">
          ...
          <div class="stat-content">
            <h3>{{ monitorStore.monitors.length }}</h3>
            <p>Total Services</p>
          </div>
        </div>
      ...
      <div v-if="filtersExpanded" class="filters">
        ...
        <div class="view-mode-toggle">
          <button 
            @click="viewMode = 'grouped'; fetchData()" 
    ...
    <!-- Empty State -->
    <div v-if="!monitorStore.loading && !monitorStore.error && (!monitorStore.monitors.length && !Object.keys(groupedMonitors).length)" class="empty-state">
...
const overallHealth = computed(() => {
  if (monitorStore.monitors.length === 0) return 0
  const upCount = upMonitors.value
  return Math.round((upCount / monitorStore.monitors.length) * 100)
})
...
async function fetchGroupedMonitors() {
  try {
    const response = await monitorStore.getGroupedMonitors()
